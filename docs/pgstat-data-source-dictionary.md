...
| WAL receiver | `pg_stat_wal_receiver` | `ClusterCollector.collectWalReceiverSnapshot` on standby | `fact.pg_wal_receiver_snapshot` | snapshot | WAL Receiver tab | standby receive/apply evidence | active, standby only; field-level coverage should be periodically verified. Known data gap: 2026-06-16 to P0-022 deploy (V067 partitions ran out) |
