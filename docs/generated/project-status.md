...
| 42 | PGSTAT-P0-035 | P1 | alerting | done | table_threshold alert message is too dense to scan when several arrive at once | Kapandı — canlı doğrulaması yapıldı, müşteri onayı alındı. |
| 43 | PGSTAT-P0-036 | P1 | alerting | done | Bloat alert follow-ups: missing API wiring, resolve notification detail, per-rule channel selection, evidence-based action text | Kapandı — canlı doğrulaması yapıldı, müşteri onayı alındı. |
...
| 48 | PGSTAT-P0-038 | P0 | alerting | done | Bloat suppression froze an already-open alert and never fired on tables whose dead tuples stopped growing | Kapandı — canlı doğrulaması yapıldı, müşteri onayı alındı. |
| 49 | PGSTAT-P0-039 | P0 | alerting | done | Granular threshold alerts collapse every offending table into one instance-level alert, so only the worst one is ever visible | Kapandı — canlı doğrulaması yapıldı, müşteri onayı alındı. |
| 50 | PGSTAT-P0-040 | P0 | alerting | done | Dead-tuple ratio is computed from n_live_tup without checking whether that estimate is trustworthy, producing false alerts on never-analyzed tables | Kapandı — canlı doğrulaması yapıldı, müşteri onayı alındı. |
| 51 | PGSTAT-P1-012 | P1 | alerting | done | No alert warns that a table statistics are stale, even though stale estimates mislead the planner and this system own diagnostics | Kapandı — canlı doğrulaması yapıldı, müşteri onayı alındı. |
...
| 59 | PGSTAT-P0-045 | P0 | collector | planned | Table sizes are only measured nightly, so a bloat alert can rest on a day-old figure and cannot clear until the next night | Awaiting customer acceptance. The fix is verified live: four source=watched rows landed at 11:00 on 2026-09-01, the first ever written. The watch list has since been narrowed to bloat alerts, which is deployed but cannot show rows until the bloat rule is re-enabled around 2026-09-28; re-verify then. |
...
| 63 | PGSTAT-P0-048 | P0 | collector | in_progress | Alerts reach Telegram that name no table, fire on trivial tables, and measure staleness from the wrong instant | Adim 2 kodlandi ve V116 gercek PG17 de dogrulandi; deploy edilebilir. Deploy sonrasi eski instance-anahtarli alarmlarin kapandigini ve yeni :rec: anahtarlarinin acildigini gor, sonra B yonu sorgusunu severity is not null ile daralt. Sirada AC6 (ACK epizoda). AYRICA INCELENECEK: 11 gunde 1296 kapanmis epizot - alarm flapping sinyali, AC8 bildirim backoff unu dogrudan ilgilendiriyor. |
...
| PGSTAT-P0-046 | - | - |
| PGSTAT-P0-047 | - | - |
| PGSTAT-P0-048 | - | PGSTAT-P1-020 |
| PGSTAT-P1-020 | PGSTAT-P0-048 | - |
