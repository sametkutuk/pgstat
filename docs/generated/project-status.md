...
| 46 | PGSTAT-P1-011 | P1 | collector | planned | Measure autovacuum's actual system cost (I/O/CPU/query latency), not just whether it ran | Implement AC2 per the design doc: fetchAutovacuumImpactOnLatency() and the IO-wait-ratio extension to fetchAutovacuumWorkerStatus(), wired into scenario 3/4.5's diagnosis text as additional evidence sentences (not new top-level scenarios). Then AC3's live verification. |
