...
| 46 | PGSTAT-P1-011 | P1 | collector | planned | Add observational autovacuum-pressure evidence (I/O operation counts, wait-event distribution) to the existing bloat alert | AC1 is complete: the design doc (docs/autovacuum-cost-diagnosis-design.md) has been consolidated across four review rounds and is now the single canonical source — every board field on this task (title, problem, requirements, AC1-3, implementation_plan, verification_plan) is synchronized with it, and superseded evidence entries are marked [SUPERSEDED] rather than silently left standing. ONE item is genuinely open: whether queryForList() returns OffsetDateTime or java.sql.Timestamp for timestamp columns (code must normalize both, test both). Everything else previously called open is now a closed decision documented in the doc: sufficiency gate = count(distinct snapshot_ts) >= 10; runningWorkers = count(distinct pid) at the instance latest OVERALL activity snapshot with a freshness check; settings reads use DISTINCT ON + ORDER BY snapshot_ts DESC + freshness; Diagnosis 0 uses a five-state model (UNSUPPORTED/UNKNOWN_CAPABILITY/NO_DATA/ZERO_WITH_FRESH_DATA/AVAILABLE); the cost_delay recommendation gate is effectiveCostDelay > versionDefault (20ms PG11, 2ms PG12+), not merely non-default. START AC2 with step 0: the 9 prerequisite code fixes in the doc Kod önkoşulları section, which include two found only in the latest round — autovacuum_worker_slots is not collected at all despite 3 PG18 fleet instances (it caps effective worker capacity below autovacuum_max_workers), and ClusterCollector getDoubleOrZero() coerces source NULLs to 0.0 for I/O counters, blocking the NO_DATA vs real-zero distinction Diagnosis 0 depends on. Also note autovacuum (the on/off parameter) is missing from BOTH settings lists, not just the hot one, while vacuum_cost_delay/vacuum_cost_limit are missing only from HOT_SETTINGS_QUERY. Byte-volume metrics and any UI chart are explicitly OUT of scope for this task (see the doc Scope kararı section) and belong to a separate future task. Diagnosis 0 numbers recorded so far predate the object=relation filter, so re-measure rather than citing them as verified. After any board edit run node scripts/check-project-board.mjs and node scripts/generate-project-status.mjs. |
