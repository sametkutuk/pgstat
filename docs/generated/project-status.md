...
| 46 | PGSTAT-P1-011 | P1 | collector | planned | Measure autovacuum's actual system cost (I/O/CPU/query latency), not just whether it ran | AC1 (design + live verification) is fully closed with no known open questions. Implement AC2 per the revised design doc: fetchAutovacuumIoImpact() (Diagnosis 0, PG16+ gated) first; fetchAutovacuumWorkerStatus()'s IO-wait-ratio and throttle-sleep-ratio extensions (Diagnoses 2/2b) second; the cost-setting-at-default guard; the latency-correlation helper (Diagnosis 1) last and gated on having ≥10 buckets on both sides. Wire into scenario 3/4.5's diagnosis text as additional evidence sentences on the existing dead_tuple_ratio alert — not a new alert type, not adaptive alerting. Then AC3's live verification on instance_pk=6/8/23. |
