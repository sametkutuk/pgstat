...
| 41 | PGSTAT-P0-034 | P0 | alerting | in_progress | dead_tuple_ratio alert rule redesigned with a 3-signal bloat/vacuum-ineffectiveness model | Deploy the DISTINCT ON fix (./pgstat upgrade); ask the customer to re-verify that a manual VACUUM now resolves the alert within one db_objects collection cycle (≤30 min) instead of up to the full evaluation window; watch for a few more days for false positives before considering this task fully accepted. |
