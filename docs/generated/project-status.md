...
| 45 | PGSTAT-P1-010 | P1 | collector | in_progress | agg.pg_table_stat_hourly / agg.pgss_hourly grow continuously due to frequent UPSERT-driven dead tuples, default autovacuum threshold too slow | Deploy the corrected V094 (reloption fix + ownership fix) + PartitionManager change + the dead_tup-ordering fix together (./pgstat migrate && ./pgstat upgrade); monitor over the next several days whether autovacuum on the current-month partitions of these two tables starts firing more often and whether their size growth rate slows; live-verify that rule:14:instance:23's next message names agg.pg_table_stat_hourly instead of control.instance_state; live-verify the next monthly partition rollover creates partitions with the lowered threshold already set. |
