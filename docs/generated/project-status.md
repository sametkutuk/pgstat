...
| 51 | PGSTAT-P1-012 | P1 | alerting | planned | No alert warns that a table statistics are stale, even though stale estimates mislead the planner and this system own diagnostics | Deploy with V101, then confirm the alert fires only where PostgreSQL own threshold is genuinely crossed - notably that t_ets_hotel_transaction_log, 29 days without ANALYZE but only 34 percent of the way to its threshold, does not appear. |
