...
| 61 | PGSTAT-P0-046 | P0 | alerting | planned | The physical bloat rule compares a size and a row count from different moments, and matches table history by name | TEMPORARY STATE: rule 176 is enabled and a synthetic probe row set (relid 999000001, schema pgstat_probe) is loaded on instance 20. Remove both once probe testing is finished - delete the probe rows, delete alerts matching pgstat_probe, and set is_enabled back to false. Real history cannot start before 2026-09-01, so the rule can only speak from 2026-09-29. Re-enable then, after checking its first output against a manual measurement. |
