...
| 49 | PGSTAT-P0-039 | P0 | alerting | planned | Granular threshold alerts collapse every offending table into one instance-level alert, so only the worst one is ever visible | Redeploy urgently: between 12:23 and this fix no granular alert could open at all. Then confirm on instance 8 that t_ets_hotel_transaction_log raises with the untrustworthy-statistics diagnosis. |
| 50 | PGSTAT-P0-040 | P0 | alerting | planned | Dead-tuple ratio is computed from n_live_tup without checking whether that estimate is trustworthy, producing false alerts on never-analyzed tables | Deploy, then confirm the five affected tables on instance 2 no longer claim ratios above 90 percent and that the ANALYZE-first diagnosis renders correctly. |
| 51 | PGSTAT-P1-012 | P1 | alerting | codex_verified | No alert warns that a table statistics are stale, even though stale estimates mislead the planner and this system own diagnostics | Awaiting customer sign-off. The rule is live and correctly silent: four tables have crossed their threshold and autoanalyze took all of them within minutes. |
