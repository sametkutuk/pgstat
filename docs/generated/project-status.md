...
| 41 | PGSTAT-P0-034 | P0 | alerting | in_progress | dead_tuple_ratio alert rule redesigned with a 3-signal bloat/vacuum-ineffectiveness model | Deploy the DISTINCT ON + cooldown/resolve fixes together (./pgstat upgrade); ask the customer to re-verify that the accounting.docs_inf_document alert (and any other currently-open granular-threshold alert) resolves promptly once the underlying data recovers, without waiting out the rule's cooldown window; watch for a few more days for false positives before considering this task fully accepted. |
