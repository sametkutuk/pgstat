...
| 41 | PGSTAT-P0-034 | P0 | alerting | in_progress | dead_tuple_ratio alert rule redesigned with a 3-signal bloat/vacuum-ineffectiveness model | Deploy the resolve->resolveAndNotify fix (./pgstat upgrade); ask the customer to confirm the next real alert resolution produces a 'Resolved: ...' notification on the delivery channel; watch for a few more days for false positives before considering this task fully accepted. |
