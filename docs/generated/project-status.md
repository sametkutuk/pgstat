...
| 46 | PGSTAT-P1-011 | P1 | collector | planned | Add observational autovacuum-pressure evidence (I/O operation counts, wait-event distribution) to the existing bloat alert | AC1 is complete and NO open design decisions remain — the design doc (docs/autovacuum-cost-diagnosis-design.md) is the single canonical source and every board field on this task is synchronized with it; superseded evidence entries are marked [SUPERSEDED]. Five review rounds have closed every contract AC2 needs: (a) the worker-evidence record carries FOUR independent status fields (currentWorkerStatus / ioWaitStatus / throttleStatus / capacityStatus) plus workerSlots and effectiveWorkerCapacity — a single enum provably cannot represent PG12 or a short-sample window; (b) Diagnosis 0 has a six-state ordered model whose NO_FRESH_DATA decision comes from source_row_count and latest_sample_ts, not from counter values, and whose zero state is named ZERO_IO_WITH_FRESH_DATA because zero relation I/O does not mean autovacuum did not run; (c) the typed I/O counter refactor is mandatory prerequisite 10, not optional, since three other prerequisites mutate the same cache; (d) the effective-settings chain is written step-by-step per parameter with an UNKNOWN outcome that suppresses the cost_delay recommendation, and all freshness thresholds derive from collector cadence x2 + grace rather than hardcoded hours. START AC2 with step 0: the 10 prerequisite code fixes in the design doc Kod önkoşulları section. Note two that are easy to misread: the autovacuum on/off parameter is missing from BOTH settings lists (not just the hot one), while vacuum_cost_delay/vacuum_cost_limit are missing only from HOT_SETTINGS_QUERY; and autovacuum_worker_slots (PG18+, 3 fleet instances) is collected nowhere. Also carry the PG18 balancing exception into any capacity wording: workers processing tables that have per-table cost storage parameters are excluded from cost balancing entirely, which is exactly the case this feature reads overrides for, so avoid per-worker budget arithmetic and report observed ratios instead. Byte-volume metrics and UI charts remain OUT of scope (see the doc Scope kararı section). Diagnosis 0 figures recorded so far predate the object=relation filter — re-measure in AC3 rather than citing them. After any board edit run node scripts/check-project-board.mjs and node scripts/generate-project-status.mjs. |
