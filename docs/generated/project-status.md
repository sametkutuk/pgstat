...
| 49 | PGSTAT-P0-039 | P0 | alerting | planned | Granular threshold alerts collapse every offending table into one instance-level alert, so only the worst one is ever visible | Deploy with the V098 migration, then verify on instance 2 that the four tables above threshold each get their own alert, that a single summary notification was delivered rather than four, and that vacuuming one resolves only its own alert. |
