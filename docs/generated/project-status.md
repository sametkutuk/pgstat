...
| 30 | PGSTAT-P0-024 | P0 | operations | in_progress | Raise a system alert when a ready instance loses pg_hba access (degrades silently today) | Deploy to production; live-verify by revoking pg_hba access for a test instance and confirming a Telegram alert fires, then restoring access and confirming auto-resolve. Close AC3 and this task after that verification. |
