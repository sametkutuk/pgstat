...
| 46 | PGSTAT-P1-011 | P1 | collector | planned | Measure autovacuum's actual system cost (I/O/CPU/query latency), not just whether it ran | STATUS NOTE (2026-08-26): docs/autovacuum-cost-diagnosis-design.md was fully rewritten/consolidated in a third review pass — prior versions had accumulated conflicting 'DÜZELTME' (correction) paragraphs layered on top of wrong original text rather than replacing it, which itself produced new inconsistencies (e.g. the doc said '23 instances' in one place and '25' via a correction note in another; Diagnosis 0 was called both 'primary' and 'PG16+ bonus' in different sections; the -1 sentinel was said to be both 'introduced in PG12' and, after a later correction, 'present since PG11' without the first claim being removed). The rewritten doc has ONE canonical explanation per topic, verified facts only: cost_delay's 20ms->2ms default drop is PG12-specific (PG13 unchanged); the -1 fallback to vacuum_cost_delay/vacuum_cost_limit is confirmed present in PG11's own docs (fetched live) and never removed through PG17; a high IO-wait ratio does NOT prove cost_delay is misconfigured (independent signals from VacuumDelay); Diagnosis 0 is BONUS evidence only, PG16+ gated, never called primary; Diagnosis 2b is PG13+ gated (VacuumDelay wait_event doesn't exist before PG13); the registered instance count is 25, not 23 (60% below PG16, not 65%); vacuum_cost_page_hit/miss/dirty are the correct unprefixed parameter names (no autovacuum_vacuum_cost_page_* exists); wait_event IS NULL means 'no wait event reported,' not 'actively running.' Three items are explicitly left OPEN for AC2, not resolved: the >=10-sample gate is now DEFINED as count(distinct snapshot_ts) >= 10 (closed decision, unlike earlier drafts that called it ambiguous); the queryForList() timestamp-type question remains genuinely open (must be tested, both OffsetDateTime and java.sql.Timestamp handled safely); AC3's target instances aren't fixed since pg_major can change (instance_pk=23 already moved PG15->PG17 mid-task). Before writing any Diagnosis 0/2/2b code: land the 6 confirmed prerequisite fixes in the design doc's 'Kod önkoşulları' section (distinct-pid worker counting, relid in findBloatedTables, a new migration for parsed cost-override columns, template-fallback evidence preservation, stats_reset-aware IO delta caching, and adding autovacuum/vacuum_cost_delay/vacuum_cost_limit to HOT_SETTINGS_QUERY — currently only in the nightly SETTINGS_QUERY). Then implement per the design doc's 'AC2 implementasyon sırası' section: fetchAutovacuumWorkerStatus() extended for Diagnoses 2/2b (single round-trip); effective-cost-setting helper (table override -> instance setting -> -1 fallback, gate on effectiveCostDelay > versionDefault, not just 'non-default' — 0ms/1ms are non-default but should not trigger the recommendation); fetchAutovacuumIoImpact() for Diagnosis 0 (object='relation' filter, client-reads=0 guard, no-data/zero/unsupported distinction); wire into scenario 3/4.5's evidence text on the existing dead_tuple_ratio alert (AlertCode.USER_DEFINED_RULE) — not a new alert type, not adaptive alerting. Add the unit tests enumerated in the design doc's final section. Then AC3's live verification (re-query control.instance_capability at that time to pick current sub-PG16 targets), mvn compile && mvn test, and make verify if present. Run node scripts/check-project-board.mjs and node scripts/generate-project-status.mjs after any further board edits. |
