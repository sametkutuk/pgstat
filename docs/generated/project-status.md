...
| 46 | PGSTAT-P1-011 | P1 | collector | planned | Measure autovacuum's actual system cost (I/O/CPU/query latency), not just whether it ran | Implement AC2 per the revised design doc: fetchAutovacuumIoImpact() (Diagnosis 0, primary — I/O read/write counts by backend_type) first; fetchAutovacuumWorkerStatus()'s IO-wait-ratio extension (Diagnosis 2) second; the latency-correlation helper (Diagnosis 1) last and gated on having ≥10 buckets on both sides. Wire into scenario 3/4.5's diagnosis text as additional evidence sentences. Then AC3's live verification. |
