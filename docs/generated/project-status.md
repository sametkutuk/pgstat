...
| 59 | PGSTAT-P0-045 | P0 | collector | planned | Table sizes are only measured nightly, so a bloat alert can rest on a day-old figure and cannot clear until the next night | Deploy the ClassCastException fix, then confirm fact.pg_relation_size_snapshot gains source=watched rows within 30 minutes. After that holds, narrow the watch list to bloat alerts only, which cannot be done first because the bloat rule is disabled and the list would go empty, leaving the fix unverifiable. |
