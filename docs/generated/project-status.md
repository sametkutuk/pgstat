...
| 63 | PGSTAT-P0-048 | P0 | collector | in_progress | Alerts reach Telegram that name no table, fire on trivial tables, and measure staleness from the wrong instant | Adim 2 kodlandi (bayat istatistik tablo basina + kidem saati epizottan). V116 KONTEYNERDE DOGRULANMADI (Docker kapaliydi) - deploy oncesi dogrulanmali. Deploy sonrasi: eski instance-anahtarli alarmlarin kapandigi, yeni :rec: anahtarlarinin acildigi ve B yonu sorgusunun daraltilmis halinin sifir dondugu gorulmeli. Sonra AC6 (ACK epizoda). |
