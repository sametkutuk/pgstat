...
| 40 | PGSTAT-P0-033 | P0 | operations | in_progress | TextEnricher's pg_stat_statements(true) call materializes the whole view, spilling to temp files | Deploy to production (./pgstat upgrade); monitor postgres logs on the two originally-affected instances plus the newly-identified high-volume instances (etscore-dev-test-253.50 etc.) for several days; re-run the fleet-wide fact.pgss_delta query periodically to confirm temp_blks_written trends to zero fleet-wide before closing AC7 and the overall task. |
